...
class RestauranteApp:
        ...
        try:
            if os.path.exists("pedidos.json"):
                with open("pedidos.json", "r", encoding="utf-8") as f:
                    dados = json.load(f)
                    # Converter chaves para inteiro se estiverem como string
                    pedidos = {}
                    for key, value in dados.items():
                        pedidos[int(key)] = value
                    return pedidos
    ...
    def setup_sidebar(self, parent):
        ...
        menu_buttons = [
            ...
            ("📦 Pedidos Ativos", self.mostrar_pedidos_ativos),
    ...
    def novo_pedido(self):
        ...
        # Botão SALVAR
        salvar_btn = tk.Button(btn_frame, text="💾 Salvar Pedido", 
                              command=self.salvar_pedido_atual,
        ...
        # Botão LIMPAR
        limpar_btn = tk.Button(btn_frame, text="🗑️ Limpar Pedido", 
    ...
    def salvar_pedido_atual(self):
        """Salva o pedido atual como rascunho"""
        try:
            if not self.pedido_atual:
                messagebox.showwarning("Aviso", "Adicione itens ao pedido antes de salvar!")
                return
            ...
            # Salvar pedido com status "rascunho"
            ...
            if self.salvar_pedidos():
                messagebox.showinfo("Sucesso", 
                                  f"Pedido #{numero_pedido} salvo como rascunho!\n"
                                  f"Mesa: {mesa}\n"
                                  f"Status: Rascunho\n"
                                  f"Total: R$ {self.pedidos[numero_pedido]['total']:.2f}")
    ...
    def atualizar_pedido_tree(self):
        ...
        if self.pedido_atual:
            ...
            self.pedido_tree.tag_configure("total", background="#34495e", foreground="white")
    ...
    def mostrar_pedidos_ativos(self):
        """Exibe todos os pedidos ativos (rascunhos e pendentes)"""
        self.clear_content()
        self.content_title.configure(text="📦 Pedidos Ativos")
        
        pedidos_ativos = {num: ped for num, ped in self.pedidos.items() 
                         if ped["status"] in ["rascunho", "pendente"]}
        
        if not pedidos_ativos:
            self.content_text.insert("1.0", "📭 Nenhum pedido ativo!")
            return
        
        texto = f"Total de pedidos ativos: {len(pedidos_ativos)}\n\n"
        
        for numero, pedido in pedidos_ativos.items():
            status_emoji = "📝" if pedido["status"] == "rascunho" else "⏳"
            texto += f"{status_emoji} Pedido #{numero} - Mesa {pedido['mesa']} - {pedido['status'].upper()}\n"
            texto += f"🕒 Horário: {pedido['data_hora']}\n"
            texto += f"💰 Total: R$ {pedido.get('total', 0):.2f}\n"
            texto += "Itens:\n"
            
            total_pedido = 0
            for codigo, quantidade in pedido["itens"].items():
                item = self.cardapio[int(codigo)]  # Converter para int
                subtotal = item["preco"] * quantidade
                total_pedido += subtotal
                texto += f"  [{quantidade:2d}x] {item['nome']} - R$ {subtotal:.2f}\n"
            
            texto += f"📋 Total do pedido: R$ {total_pedido:.2f}\n"
            texto += "-" * 50 + "\n\n"
        
        self.content_text.insert("1.0", texto)
        
        # Botão para atualizar
        atualizar_btn = tk.Button(self.action_frame, text="🔄 Atualizar Lista",
                                command=self.mostrar_pedidos_ativos,
    ...
    def gerar_comanda(self):
        ...
        try:
            ...
            for codigo, quantidade in pedido["itens"].items():
                categoria = self.cardapio[int(codigo)]["categoria"]
    ...
    def mostrar_relatorio(self):
        ...
        pedidos_rascunho = {num: ped for num, ped in self.pedidos.items() 
        ...
        texto += "=" * 60 + "\n\n"
        texto += "📋 RESUMO:\n"
        texto += f"Pedidos Rascunho: {len(pedidos_rascunho)}\n"
        ...
        if pedidos_rascunho:
            texto += "📝 PEDIDOS RASCUNHO:\n"
            for numero, pedido in pedidos_rascunho.items():
